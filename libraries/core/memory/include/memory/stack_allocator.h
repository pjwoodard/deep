...
class StackAllocator
{
  public:
    std::span<std::byte> allocate(size_t size)
    {
        ...
        {
            deep::Logger::assert_and_log(
              fmt::format("StackAllocator ran out of memory. Requested {} bytes, but only {} bytes are available",
                size,
                pool_size_ - stack_marker_),
              false);
        }
    ...
    void deallocate(std::span<std::byte> /*memory*/) { stack_marker_ = prev_stack_marker_; };
